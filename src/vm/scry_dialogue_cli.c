...
static bool scry_dialogue_step_compare(scry_vm* vm, FILE* input, FILE* output)
{
	...
	if (!scry_sdlg_read_uvarint(vm->view->instructions.data, vm->view->instructions.size, &cursor, &variable_id) ||
		...
		!scry_sdlg_read_svarint(vm->view->instructions.data, vm->view->instructions.size, &cursor, &compare_value) || variable_id > UINT32_MAX ||
		compare_op > UINT32_MAX || compare_value < INT32_MIN || compare_value > INT32_MAX)
	{
		scry_dialogue_fault(vm, SCRY_VM_FAULT_TRUNCATED_INSTRUCTION);
		return false;
	}

	if (snprintf(prompt, sizeof(prompt), "variable[%" PRIu64 "]> ", variable_id) < 0 || !scry_dialogue_prompt_number(input, output, prompt, &variable_value) ||
...
static bool scry_dialogue_step_choice(scry_vm* vm, FILE* input, FILE* output)
{
	...
	if (!scry_sdlg_read_uvarint(vm->view->instructions.data, vm->view->instructions.size, &cursor, &table_index) || table_index > UINT32_MAX ||
...

	{
		...
		if (!scry_dialogue_read_choice_option(&payload, selected_option, &option) || !scry_dialogue_set_ip(vm, cursor, option.target_delta))
		{
...
static bool scry_dialogue_step_switch(scry_vm* vm, FILE* input, FILE* output)
{
	...
	if (!scry_sdlg_read_uvarint(vm->view->instructions.data, vm->view->instructions.size, &cursor, &table_index) || table_index > UINT32_MAX ||
		...
		!scry_dialogue_read_u16_le(payload.data, payload.size, 0U, &case_count) || !scry_dialogue_read_u16_le(payload.data, payload.size, 2U, &variable_id) ||
	...
	if (fprintf(output, "switch variable[%" PRIu16 "]\n", variable_id) < 0 || !scry_dialogue_prompt_number(input, output, "variable> ", &variable_value))
	{
		scry_dialogue_fault(vm, SCRY_VM_FAULT_HOST_REJECTED);
		return false;
	}
