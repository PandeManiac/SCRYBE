...
static size_t scry_sdlg_compile_block_size(
	const scry_ir_node* node,
	uint32_t table_index,
	size_t primary_jump_operand_size,
	size_t secondary_jump_operand_size);
static bool scry_sdlg_compile_write_instructions(
	uint8_t* buffer,
	size_t buffer_size,
	size_t instruction_offset,
	const scry_ir_program* program,
	const scry_sdlg_compile_layout* layout);
static bool scry_sdlg_compile_write_tables(
	uint8_t* buffer,
	size_t buffer_size,
	size_t table_offset,
	const scry_ir_program* program,
	const scry_sdlg_compile_layout* layout);
static bool scry_sdlg_compile_write_strings(
	uint8_t* buffer,
	size_t buffer_size,
...
bool scry_sdlg_write_ir_file(const char* path, const scry_ir_program* program, const scry_sdlg_string* strings)
{
	...
	ok = scry_file_write_bytes(path, binary.data, binary.size);
...
static bool scry_sdlg_compile_compute_layout(const scry_ir_program* program, const scry_sdlg_string* strings, scry_sdlg_compile_layout* layout)
{
	...
	for (uint32_t i = 0U; i < program->node_count; ++i)
	{
		...
		{
			layout->table_payload_size += sizeof(scry_sdlg_choice_table_header) +
										  ((size_t)node->data.choice.option_count * (sizeof(uint32_t) + sizeof(int32_t)));
		}

		else if (node->kind == SCRY_IR_NODE_KIND_SWITCH)
		{
			layout->table_payload_size += sizeof(scry_sdlg_switch_table_header) +
										  ((size_t)node->data.switch_branch.case_count * (sizeof(int32_t) + sizeof(int32_t)));
		}

		layout->block_sizes[i] =
			scry_sdlg_compile_block_size(
				node,
		...
		for (uint32_t i = 0U; i < program->node_count; ++i)
		{
			...
			switch (node->kind)
			{
				case SCRY_IR_NODE_KIND_DIALOGUE:
				{
					...
					const size_t jump_base =
						block_offset + dialogue_size + 1U + layout->primary_jump_operand_sizes[i];
					const int64_t delta = scry_sdlg_compile_relative_delta(
						layout->block_offsets[node->data.dialogue.next_node_index],
						jump_base);
					const size_t new_size = scry_sdlg_svarint_size(delta);
				...
				case SCRY_IR_NODE_KIND_JUMP:
				{
					...
					const int64_t delta = scry_sdlg_compile_relative_delta(
						layout->block_offsets[node->data.jump.target_node_index],
						jump_base);
					const size_t new_size = scry_sdlg_svarint_size(delta);
				...
				case SCRY_IR_NODE_KIND_BRANCH:
				{
					...
					const size_t true_base =
						false_base + 1U + layout->secondary_jump_operand_sizes[i];
					const int64_t false_delta =
						scry_sdlg_compile_relative_delta(layout->block_offsets[node->data.branch.false_node_index], false_base);
					const int64_t true_delta =
						scry_sdlg_compile_relative_delta(layout->block_offsets[node->data.branch.true_node_index], true_base);
	...
	switch (node->kind)
	{
		...
		case SCRY_IR_NODE_KIND_BRANCH:
			return 1U + scry_sdlg_uvarint_size(node->data.branch.condition.variable_id) +
				   scry_sdlg_uvarint_size((uint64_t)node->data.branch.condition.compare_op) +
				   scry_sdlg_svarint_size(node->data.branch.condition.value) + 1U + primary_jump_operand_size + 1U +
				   secondary_jump_operand_size;
		case SCRY_IR_NODE_KIND_CHOICE:
		...
		switch (node->kind)
		{
			case SCRY_IR_NODE_KIND_DIALOGUE:
			{
				...
				if (!scry_sdlg_write_svarint(
						...
						buffer_size,
						&absolute_offset,
						scry_sdlg_compile_relative_delta(
							layout->block_offsets[node->data.dialogue.next_node_index],
			...
			case SCRY_IR_NODE_KIND_BRANCH:
				...
				if (!scry_sdlg_write_svarint(
						...
						&absolute_offset,
						scry_sdlg_compile_relative_delta(
							layout->block_offsets[node->data.branch.false_node_index],
							cursor + layout->primary_jump_operand_sizes[i])))
				{
					return false;
				}
				...
				if (!scry_sdlg_write_svarint(
						...
						&absolute_offset,
						scry_sdlg_compile_relative_delta(
							layout->block_offsets[node->data.branch.true_node_index],
	...
	const scry_sdlg_compile_layout* layout)
{
	...
	size_t payload_offset =
		table_offset + ((size_t)layout->table_count * sizeof(scry_sdlg_table_entry));
	...
	for (uint32_t i = 0U; i < program->node_count; ++i)
	{
		...
		if (node->kind == SCRY_IR_NODE_KIND_CHOICE)
		{
			if (!scry_sdlg_compile_write_u16_le(
					buffer, buffer_size, &payload_offset, (uint16_t)node->data.choice.option_count))
			{
				return false;
			}
			...
			{
				const scry_ir_choice_option* option =
					&program->choice_options[node->data.choice.option_start + option_offset];

				if (!scry_sdlg_compile_write_u32_le(buffer, buffer_size, &payload_offset, option->text_string_id) ||
					!scry_sdlg_compile_write_s32_le(
						buffer,
						buffer_size,
						&payload_offset,
						(int32_t)scry_sdlg_compile_relative_delta(
							layout->block_offsets[option->target_node_index],
		...
		{
			if (!scry_sdlg_compile_write_u16_le(
					buffer, buffer_size, &payload_offset, (uint16_t)node->data.switch_branch.case_count) ||
				!scry_sdlg_compile_write_u16_le(
					buffer, buffer_size, &payload_offset, (uint16_t)node->data.switch_branch.variable_id) ||
				!scry_sdlg_compile_write_s32_le(
					buffer,
					buffer_size,
					&payload_offset,
					(int32_t)scry_sdlg_compile_relative_delta(
						layout->block_offsets[node->data.switch_branch.default_node_index],
						layout->block_offsets[i] + layout->block_sizes[i])))
			{
				return false;
			}
			...
			{
				const scry_ir_switch_case* switch_case =
					&program->switch_cases[node->data.switch_branch.case_start + case_offset];
				...
				if (!scry_sdlg_compile_write_s32_le(buffer, buffer_size, &payload_offset, switch_case->value) ||
					!scry_sdlg_compile_write_s32_le(
						...
						&payload_offset,
						(int32_t)scry_sdlg_compile_relative_delta(
							layout->block_offsets[switch_case->target_node_index],
							layout->block_offsets[i] + layout->block_sizes[i])))
				{
					return false;
				}
	...
	const scry_sdlg_string* strings)
{
	...
	for (uint32_t i = 0U; i < program->string_count; ++i)
	{
		...
		{
			if (memchr(strings[i].data, '\0', strings[i].length) != NULL ||
				data_offset > buffer_size || strings[i].length > (buffer_size - data_offset))
			{
				return false;
			}
